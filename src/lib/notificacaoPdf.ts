...
export async function gerarPdfNotificacao(args: {
  ...
  const { notificacao, unidade, pessoa, condominio } = args
  ...
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(14)
  doc.text('NOTIFICAÇÃO / ADVERTÊNCIA', 105, y, { align: 'center' })
  doc.setFontSize(10)
  doc.setFont('helvetica', 'normal')
  doc.text(`Nº ${notificacao.id.slice(0, 8).toUpperCase()} · ${fmtData(notificacao.created_at)}`, 105, y + 6, { align: 'center' })
  y += 16
  ...
  doc.setFont('helvetica', 'bold')
  doc.text('Assunto', 20, y); y += 6
  doc.setFont('helvetica', 'normal')
  y = escreverBloco(doc, notificacao.assunto, 20, y, W - 40, 5)
  y += 4

  doc.setFont('helvetica', 'bold')
  doc.text('Descrição', 20, y); y += 6
  doc.setFont('helvetica', 'normal')
  y = escreverBloco(doc, notificacao.descricao || '—', 20, y, W - 40, 5)
  y += 4

  if (notificacao.artigo_regimento) {
    doc.setFont('helvetica', 'bold')
    doc.text('Base no regimento interno', 20, y); y += 6
    doc.setFont('helvetica', 'normal')
    y = escreverBloco(doc, notificacao.artigo_regimento, 20, y, W - 40, 5)
    y += 4
  }

  // Caixa de aviso (sem valor monetário, mas chama atenção)
  doc.setFillColor(254, 243, 199) // amarelo claro
  doc.rect(20, y, W - 40, 18, 'F')
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(11)
  doc.setTextColor(146, 64, 14)
  doc.text('⚠ Esta é uma advertência formal', 25, y + 7)
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(9)
  doc.text('Reincidência poderá resultar em multa pecuniária conforme regimento.', 25, y + 13)
  doc.setTextColor(20)
  y += 24
  ...
  const corpo = [
    ...
    'Esta notificação tem caráter administrativo e busca preservar a boa convivência entre condôminos. Em caso de dúvida ou contestação, procure a administração no prazo de 15 (quinze) dias contados da ciência.',
  ]
  ...
  doc.text(
    `Documento gerado eletronicamente pelo OnWay Condomínio · ${fmtData(new Date().toISOString())} · ID ${notificacao.id}`,
